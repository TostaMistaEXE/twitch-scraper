...
for ($i = 0; $i <= count($streamers) - 1; ++$i) {
    ...
    if ($pid == -1) {
        ...
        $request = new RequestFactory::create($GLOBALS['streamer'], '1', 'status');
        $request = new RequestFactory::create($GLOBALS['streamer'], null, 'checkTwitchOnline');

        if (empty($request)) {
            $request = new RequestFactory::create($GLOBALS['streamer'], '0');
            die();
        }

        $request = new RequestFactory::create($GLOBALS['streamer'], '1', 'online');
